...
if rubro == "Seleccione":
    ...
    with col2:
        ...
        e = st.number_input("Valor del inventario (Lps)", 1, 10000000000000, 20000)
    ...
    a = st.number_input("Cuota mensual (Lps)", 0, 10000000000, 5000)
    b = st.number_input("Tasa anual de interés %", 0, 100, 18)
    c = st.number_input("periodo de amortización (meses)", 0, 100, 15)
    d = (a * ((1 + b/1200)**c - 1)) / (b/1200 * (1 + b/1200)**c)
    st.write("##### :blue[Resultado del cálculo: Monto total disponible para el crédito (Lps):]")
    st.text(round(d))
    ...
    st.write("###### :blue[Se puede calcular la meta de venta, en base al análisis del punto de equilibrio. Este calculadora se aplica con facilidad en diferentes negocios incluyendo la panadería.]")  
    a = st.number_input("Precio unitario (¿cuánto cuesta un paquete de panes a vender como promedio?)", 1, 100000000000, 50)
    b = st.number_input("Costo variable unitario (¿cuánto vale el costo de materias primas para un paquete?)", 0, 100000000000, 15)
    c = st.number_input("Costo fijo mensual (alquiler del espacio, depreciación de la maquina, costo de electricidad, costo de trabajadores, etc.)", 1, 100000000000, 30000)
    d = st.number_input("Ganancias mensuales que desea (Lps)", 1, 10000000000, 5000)
    CM = a-b
    CMR = CM/a

    st.write("##### Resultado del cálculo: Monto de la venta necesaria para alcanzar la ganancia deseada (Lps)")
    ...
    d = st.number_input("Promedio estimado de la venta por cliente (Lps)", 1, 3000, 120)
    ...
    with col2:
        ...
        st.write("###### Ciertas empresas de imresión necesitan inversiones de (i) computadoras para diseño gráficos, y (ii) máquina de imprenta para diferentes materiales. El equipo de imprenta seria apropiado probablemente para aplicar leasing.")
        st.write("###### La cooperativa deberá analizar si la inversión no genera complicaciones en el flujo de caja del negocio o no.")
